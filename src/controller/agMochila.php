...
class ControllerAlgoritmoGeneticoMochila {
    ...
    public function __construct() {
        ...
        $this->sequencialGeracaoCriterioParada = 20;
        $this->geracao                         = [];
    ...
    private function processaCriacaoInformacaoGraficaGeracao() {
        $sMensagem = '<hr><div><center><h3 align-content: center>Geração: '. $this->sequencialGeracao . '</h2>';
        foreach ($this->geracao['populacao'] as $iKey => $aCromossomo) {
            $iIndiceCromossomo = $iKey + 1;
            $sMensagem .= '<center><h4>Cromossomo: ' . $iIndiceCromossomo . '</h4>'; 
            foreach ($aCromossomo as $aGene) {
                if ($aGene['peso']) {
                    $sMensagem .= '  * ' . ($aGene['peso'] / $aGene['tipoGene']->getPeso()) . ' do tipo ' . $aGene['tipoGene']->getNome() . '</br>';
                }
            }
            $sMensagem .= ' <center> * Peso Total na Mochila: ' . $this->getPesoCromossomo($aCromossomo) . '</br>  * Valor Total na Mochila: ' . $this->getValorCromossomo($aCromossomo) . '</br></div>';
        }
        echo $sMensagem;
    }
    ...
    private function processaCriacaoInformacaoGraficaFinal() {
        $sMensagem = '<hr><h3><center>O maior valor: R$' . $this->getValorCromossomo($this->bestOf['cromossomo']) . '. Gerado pela primeira vez na geração ' . $this->bestOf['sequencialGeracao'] . '. Sendo estes, valores dos seguintes objetos: </br></br>';
